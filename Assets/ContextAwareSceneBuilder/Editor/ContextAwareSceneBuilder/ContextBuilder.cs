...
namespace ContextAwareSceneBuilder.Editor
{
    ...
    public static class ContextBuilder
    {
        ...
        public static string BuildContextPack(string userPrompt, int tokenBudget, out string systemMessage, out string toolsJson)
        {
            ...
            sysSb.AppendLine("• NEVER skip scale multiplication - even if scale is [1,1,1], show the multiplication");
            sysSb.AppendLine("• NEVER use centerOffset/heuristics if semantic points exist");
            ...
            sysSb.AppendLine("- centerOffset: Vector from pivot to visual center. Formula: visualCenter = position + centerOffset");
            ...
            sysSb.AppendLine();
            sysSb.AppendLine("FALLBACK (only when semantic points are missing):");
            sysSb.AppendLine("- ONLY use centerOffset + size fallback if one or both prefabs LACK the needed semantic point in Prefab Catalog");
            sysSb.AppendLine("- Check the Prefab Catalog's semanticPoints array - if the point name isn't listed, it's missing");
            sysSb.AppendLine("- Example: If Wall1m has no semanticPoints array or doesn't list \"back\", use centerOffset fallback");
            sysSb.AppendLine("- NEVER use fallback when both objects have the semantic points you need");
            sysSb.AppendLine();
