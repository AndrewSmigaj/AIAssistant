...
namespace ContextAwareSceneBuilder.Editor
{
    ...
    public static class ContextBuilder
    {
        ...
        public static string BuildContextPack(string userPrompt, int tokenBudget, out string systemMessage, out string toolsJson)
        {
            ...
            sysSb.AppendLine();
            sysSb.AppendLine("[ ] Step 0: WRITE DOWN all scales BEFORE any calculations (CRITICAL)");
            sysSb.AppendLine("    - For NEW object: Retrieve prefab's default scale from Prefab Catalog metadata");
            sysSb.AppendLine("    - For TARGET object: Copy scale from Scene Context (or Prefab Catalog if not in scene)");
            sysSb.AppendLine("    - WRITE EXPLICITLY: \"TableSquareMedium scale = [0.341, 0.341, 0.341]\"");
            sysSb.AppendLine("    - ONLY override new object's scale if user explicitly specifies different scale");
            sysSb.AppendLine("    - Example: TableSquareMedium [0.341,0.341,0.341] with top.local Y=2.383 → world top Y=0.813, NOT 2.383");
            sysSb.AppendLine("    - ABORT if you proceed to Step 1 without writing down scales first");
            sysSb.AppendLine();
            ...
            sysSb.AppendLine("    - VERIFY rotation maps semantic point to correct world direction:");
            sysSb.AppendLine("      Example: To align bed.back (local +Z) with wall facing south (world -Z), use Y=180° so +Z→-Z");
            sysSb.AppendLine("      Example: To align bed.back (local +Z) with wall facing west (world -X), use Y=270° so +Z→-X");
            sysSb.AppendLine("    - ABORT if you skip scale multiplication!");
